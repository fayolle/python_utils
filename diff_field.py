...
def printStatistics(err_field, exact, list_nodes, list_tris):
    (l1_err, l2_err, linf_err) = computeResidual(err_field, exact, list_nodes,
                                                 list_tris)
...
def computeRelativeResidualQuadrature(err_field, exact_field, list_nodes,
                                      list_tris):
    # Compute L1, L2 and L^inf norm for err_field using quadrature
    l1_err, l2_err, linf_err = computeResidualQuadrature(
        err_field, exact_field, list_nodes, list_tris)
...
def printStatisticsQuadrature(err_field, exact_field, list_nodes, list_tris):
    (l1_err, l2_err,
     linf_err) = computeResidualQuadrature(err_field, exact_field, list_nodes,
                                           list_tris)
    ...
    (l1, l2, linf) = computeRelativeResidualQuadrature(err_field, exact_field,
                                                       list_nodes, list_tris)
    print("Relative residual (L1 norm): " + str(l1))
    print("Relative residual (L2 norm): " + str(l2))
    print("Relative residual (Linf norm): " + str(linf))
...
def main(args):
    ...
    bbox_scaled_diff_field = computeBBoxScaledAbsDiff(field1, field2,
                                                      vertices1)
    ...
    if save_to_file:
        ...
        bbox_scaled_diff_data = (vertices1, elements1, cell_type1,
                                 bbox_scaled_diff_field)
        field_scaled_diff_data = (vertices1, elements1, cell_type1,
                                  field_scaled_diff_field)
        relative_diff_data = (vertices1, elements1, cell_type1,
                              relative_diff_field)
